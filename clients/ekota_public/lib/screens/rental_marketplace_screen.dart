...
class _RentalMarketplaceScreenState extends State<RentalMarketplaceScreen> {
  ...
  String _search = '';
  final Set<String> _watchedListingIds = {};
  ...
  Future<void> _loadItems() async {
    ...
    try {
      ...
      final watchlist = await WatchlistService.getMyWatchlist();
      
      if (!mounted) return;
      setState(() {
        _items = items;
        _filtered = items;
        _watchedListingIds.clear();
        for (var w in watchlist) {
          _watchedListingIds.add(w['listingId']);
        }
        _isLoading = false;
      });
    } catch (e) {
                                      ...
                                      children: [
                                        ...
                                        Positioned(
                                          ...
                                          child: Container(
                                            ...
                                            decoration: BoxDecoration(
                                              color: isAvailable ? Colors.green : (item['status'] == 'RETURNED' ? Colors.blue : Colors.orange),
                                              borderRadius: BorderRadius.circular(8),
                                            ),
                                            child: Text(
                                              isAvailable ? 'Available' : (item['status'] == 'RETURNED' ? 'Returned' : 'Rented'),
                                        ...
                                        Positioned(
                                          ...
                                          child: GestureDetector(
                                            onTap: () async {
                                              ...
                                              final isWatched = _watchedListingIds.contains(listingId);
                                              
                                              setState(() {
                                                if (isWatched) {
                                                  _watchedListingIds.remove(listingId);
                                                } else {
                                                  _watchedListingIds.add(listingId);
                                                }
                                              });
                                            ...
                                            child: Container(
                                              ...
                                              child: Icon(
                                                _watchedListingIds.contains(item['listingId']) ? Icons.favorite : Icons.favorite_border,
